...
class APDUCommand {
   ...
   * @param {hsd.Input} hsdInput - hsd input primitive
   * @param {Buffer} script - input script
   * @param {Boolean} initial - indicates whether this is the initial message
   * @returns {APDUCommand}
   */

  static getInputSignature(ledgerInput, hsdInput, script, initial) {
    const cla = common.cla.GENERAL;
    const ins = common.ins.GET_INPUT_SIGNATURE;
    const p1 = initial ? 0x01 : 0x00;
    const p2 = 0x01;

    let data = script;

    if (ledgerInput != null && hsdInput != null) {
      const path = encodePath(ledgerInput.path);
      const size = path.length + hsdInput.getSize() + script.length + 12;

      // Init
      data = bufio.write(size);
