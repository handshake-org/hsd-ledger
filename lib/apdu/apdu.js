...
class APDUCommand {
   ...
   * @param {Boolean} options.network - network serialization
   * @returns {APDUCommand}
   */

  static getPublicKey(path, options) {
    const cla = common.cla.GENERAL;
    const ins = common.ins.GET_PUBLIC_KEY;
    const data = encodePath(path);
    let p1 = options.confirm ? 0x01 : 0x00;
    let p2 = options.address ? 0x02 : 0x00;
  ...
  static parseTX(data, options) {
    assert(options);
    assert(options.network);

    const commands = [];
    const datum = util.splitBuffer(data, util.MAX_TX_PACKET);
    const p1 = common.getNetworkFlag(options.network);

    commands.push(new APDUCommand({
      cla: common.cla.GENERAL,
...
class APDUResponse {
  ...
  /**
   * Decode APDU response.
   * @param {Buffer} res - raw APDU packet
   * @throws {APDUError}
   */
  ...
  static parseTX(res) {
    ...
    if (res.length > 2) {
      ...
      const status = common.status.SUCCESS;
...
/**
 * Check if buffer is a status code
 * @param {Buffer} buffer
 * @throws {APDUError}
 */
...
function validate(buffer) {
  ...
  if (code === common.status.SUCCESS)
    return;

  const message = common.errorByStatus[common.statusByVal[code]];

  if (message)
    throw new APDUError(message, code, hex);

  if (buffer[0] === common.status.INTERNAL_ERROR)
    throw new APDUError(common.errorByStatus.INTERNAL_ERROR, code, hex);

  throw new APDUError(common.errorByStatus.UNKNOWN_ERROR, code, hex);
}
