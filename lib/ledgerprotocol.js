...
class ProtocolData {
  ...
  splitBuffer(data) {
    return ProtocolData.splitBuffer(data, this.packetSize);
  }
  ...
  static splitBuffer(data, packetSize) {
    const br = bufio.reader(data);
    const messages = [];

    while(br.left() > packetSize) {
      messages.push(br.readBytes(packetSize));
    }
...
class ProtocolWriter extends ProtocolData {
  ...
  toRaw() {
    const bw = bufio.static(this.getSize());
...
class ProtocolReader extends ProtocolData {
  ...
  readFirstMessage(message) {
    const br = bufio.reader(message);

    this.enforce(message.length >= 7 + 5, 'Incorrect message length');
    this.enforce(this.finished !== true, 'Writting to finished writer');
  ...
  static fromBuffer(buffer, packetSize) {
    ...
    const messages = this.splitBuffer(buffer, packetSize);
...
exports.ProtocolWriter = ProtocolWriter;
