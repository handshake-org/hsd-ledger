...
const { MTX, Network, Script } = require('hsd');
...
class LedgerClient {
  ...
  async parseTX(mtx, ledgerInputByKey) {
    ...
    let msgs = util.splitBuffer(buf.render(), util.MAX_TX_PACKET);
    let fst = msgs.shift();
  ...
  async getInputSignature(ledgerInput, hsdInput) {
    ...
    let packet = APDUCommand.getInputSignature(ledgerInput, hsdInput, first, true);
    let res = await this.device.exchange(packet.toRaw());
