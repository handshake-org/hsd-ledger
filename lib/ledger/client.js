...
class LedgerClient {
  ...
  async getInputSignature(ledgerInput) {
    assert(this.device);
    ...
    let packet = APDUCommand.getInputSignature(ledgerInput, first, true);
    ...
    for (const msg of msgs) {
      const packet = APDUCommand.getInputSignature(null, msg, false);
      const res = await this.device.exchange(packet.toRaw());
      APDUResponse.getInputSignature(res, true);
    }

    // Send the last packet, expecting a signature to be returned.
    packet = APDUCommand.getInputSignature(null, last, false);
