...
const {HDPublicKey, Network, Script} = require('hsd');
...
class LedgerHSD {
  ...
  async signTransaction(mtx, inputs) {
    ...
    try {
      ...
      for (const {input, signature} of signatures) {
  ...
  async _getSignatures(mtx, inputs) {
    const signatures = new Array(mtx.inputs.length);
