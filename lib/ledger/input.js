...
class LedgerInput {
  ...
  fromOptions(options) {
    ...
    assert(Input.isInput(options.input), 'Input must be a hsd.Input.');

    this.input = options.input;

    const index = options.index;
    assert((index >>> 0) === index, 'Index must be a Uint8.');
    assert(index >= 0 && index < 256, 'Index must be a Uint8.');

    this.index = index;

    assert(Coin.isCoin(options.coin), 'Coin must be a hsd.Coin.');

    this.coin = options.coin;

    assert(options.redeem != null || options.publicKey != null,
      'Must include public key or redeem script.');
