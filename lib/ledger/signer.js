...
class LedgerSigner {
  ...
  fromOptions(options) {
    ...
    assert(options.mtx, 'options.mtx is required.');
    assert(options.ledger instanceof Ledger,
      'options.ledger must be an instance of Ledger.');

    this.ledger = options.ledger;

    if (Buffer.isBuffer(options.mtx)) {
      this.mtx = MTX.fromRaw(mtx);
    } else {
      this.mtx = options.mtx;
    }

    assert(MTX.isMTX(this.mtx),
      'options.mtx must be an instance of MTX or Buffer.');
  ...
  async signInput(input, index, confirm) {
    ...
    return await this.ledger.getSignature(input, index, confirm);
  }
