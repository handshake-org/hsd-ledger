...
class Ledger {
  ...
  async getPublicKey(path, confirm, network) {
    assert(this.device);
    ...
    const cmd = APDUCommand.getPublicKey(path, confirm, network);
    const buf = await this.device.exchange(cmd.toRaw());
  ...
  /**
   * Send the transaction details to the Ledger device so it
   * can parse them and prepare the signature hash.
   * @async
  ...
  async getSignature(input, index, confirm = true) {
    assert(this.device);
    ...
    let packet = APDUCommand.getSignature(input, index, first, confirm);
    let res = await this.device.exchange(packet.toRaw());

    if (msgs.length == 0) {
      const { data } = APDUResponse.getSignature(res);
    ...
    APDUResponse.getSignature(res, true);

    const last = msgs.pop();

    for (const msg of msgs) {
      const packet = APDUCommand.getSignature(null, null, msg, confirm);
      const res = await this.device.exchange(packet.toRaw());
      APDUResponse.getSignature(res, true);
    }

    packet = APDUCommand.getSignature(null, null, last, confirm);
