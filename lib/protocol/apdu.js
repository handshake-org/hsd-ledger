...
const errorMessages = {
  CANNOT_ENCODE_ADDRESS: 'Cannot encode bech32 address',
  ...
  INCORRECT_WRITE_LEN: 'Incorrect write len',
...
apdu.errorByStatus = pairObjects(apdu.STATUS_WORD, errorMessages);
...
apdu.statusByVal = reverse(apdu.STATUS_WORD);
...
class APDUCommand {
   ...
   * @param {String} network - Handshake network type
   * @returns {APDUCommand}
   */

  static getPublicKey(path, confirm = 0, network = 'main') {
    const cla = apdu.cla.GENERAL;
    const ins = apdu.ins.GET_PUBLIC_KEY;
    const p1 = confirm;
    const p2 = apdu.network[network];
  ...
  static getSignature(input, index, script, confirm) {
    let data;
...
class APDUResponse {
  ...
  static getPublicKey(res) {
    ...
    const br = bufio.read(res);
    const publicKey = br.readBytes(33);
    const chainCode = br.readBytes(32);
    const addr = br.readBytes(42);
    const address = addr.toString();
    const status = br.readU16BE();
    const data = { publicKey, chainCode, address };
  ...
  static parseTX(data) {
    ...
    return emptyResponse(apdu.ins.GET_SIGNATURE);
  }
...
function emptyResponse(type) {
  return new APDUResponse({
    data: apdu.EMPTY,
    status: apdu.STATUS_WORD.SUCCESS,
    type: type
  });
}
...
function validate(statusCode) {
  ...
  if (statusNo === apdu.STATUS_WORD.SUCCESS)
    return;
  ...
  if (statusCode[0] === apdu.STATUS_WORD.INTERNAL_ERROR)
