  ...
  set(options) {
    ...
    if (options.device != null) {
      if (typeof options.device === 'string') {
        this.device = new HID.HID(options.device);
      }

      if (options.device instanceof DeviceInfo) {
        this.device = new HID.HID(options.device.path);
      }

      assert(this.device, 'Couldn\'t set device');
    }
  }
  ...
  static getDevices() {
    ...
    for (const device of allDevices) {
      if (HIDDeviceInfo.isLedgerDevice(device)) {
        devices.push(new HIDDeviceInfo(device));
      }
    }
...
class HIDDeviceInfo extends DeviceInfo {
  ...
  set(options) {
    ...
    if (options.productId != null) {
      ...
      this.vendorId = options.productId;
    }
  ...
  static isLedgerDevice(device) {
    ...
    if (device.vendorId === 0x2581 && device.productId === 0x3b7d)
      return true;
