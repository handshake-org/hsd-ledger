...
class LedgerHSD {
  ...
  async getAccountXpub(account, confirm) {
    ...
    const conf = confirm ? LedgerHSD.CONFIRM_ADDRESS : LedgerHSD.NO_CONFIRM;
    const purp = util.BIP44_PURPOSE;
    const type = util.BIP44_COIN_TYPE[this.network.type];

    const purpose = util.harden(purp);
    const coinType = util.harden(type);
    const acct = util.harden(account);

    // TODO(boymanjor): return parent pubkey from apdu command
    const path = [purpose, coinType, acct];
    const parent = await this.ledger.getPublicKey(path.slice(0, 2));
    const child = await this.ledger.getPublicKey(path, conf);
    const fp = hash160.digest(parent.publicKey);
    const parentFingerPrint = fp.readUInt32BE(0, true);
  ...
  async getXpub(path) {
    ...
    // TODO(boymanjor): return proper parent fingerprint
    return new HDPublicKey({
      depth: path.length,
      childIndex: path[path.length - 1],
      parentFingerPrint: 0,
  ...
  async getAddress(account, branch, index, confirm) {
    ...
    const path = [purpose, coinType, acct, branch, index];
    const data = await this.ledger.getPublicKey(path, conf);
  ...
  async getPublicKey(account, branch, index, confirm) {
    ...
    const conf = confirm ? LedgerHSD.CONFIRM_PUBLIC_KEY : LedgerHSD.NO_CONFIRM;
    const purp = util.BIP44_PURPOSE;
    const type = util.BIP44_COIN_TYPE[this.network.type];
