...
class LedgerBcoin {
  ...
  async hashTransactionStart(tx, inputKey, prev, tis = {}, isNtx = false) {
    ...
    for (const input of ntx.inputs) {
      ...
      const buffer = bufio.static(2 + tis[pokey].length + scriptVarintSize);
...
class SignInput {
  ...
  set(options) {
    ...
    assert(options.index, 'Output index is required.');
    assert(isU32(options.index), 'Output index must be a uint32.');
    this.index = options.index;

    if (options.type != null) {
      assert(util.hashTypeByVal[options.type],
